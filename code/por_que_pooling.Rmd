...
el número de familas por edificio es $2$ y el número de de edificios por 
...
que pertenece el individuo $j$, el porcentaje todal de contagiados $p_{0_i}$ y 
ciertos pesos $\alpha_x$ para así determinar la probabilidad de que el el 
...
indivíduo hasta la última iteración será como contagiado. Nótese que en este
...
$m$ cantidad de invidíduos en un grupo y detectar ausencia y presencia del
virus. Este procedimiento es particularmente efectivo cuando se pueden descartar
grandes cantidades de invidíduos, es decir, cuando la probabilidad de haya un
indivíduo infectado en cada grupo es baja. De hecho. si esta probabilidad es 
...
construída para determinar si uso de *pooling* para reducir el número total 
de pruebas necesarias para identificar los casos positivos $T$ . 
Si suponemos que sólamente se efectuará
...
población con una proporción constante de infectados $p0$. Para ello probaremos
desde grupos de 2 hasta 64 personas (referenciando el artículo anteriormente
mencionado). Para evaluar la intuición de que colocar indivíduos con un nivel
similar de probabilidad de estar infectados o bien con cierta cercanía aumenta
la efectividad del procedimiento de *pooling* se ordenan los valores por familia
por edificio, vencindad y también por probabilidad de estar infectados.
...
Otro punto importante de tratar es que que estamos usando la variable $p_{ij}$ 
...
a la probabilidad real de que un indivíduo esté infectado, por lo que los 
resultados deben ser interpretados con precaución. Cabe resaltar que en este
caso al ser una variable contínua realmente estamos *ordenando* según $p_{ij}$
para luego *recortar* esa poblacion ordenada.
...
condición de analizar a todos los invidíduos. Puede observarse que el caso base 
...
procedimiento de *pooling*, hay que comprobar que estos hayazgos se mantienen
...
dode se obtiene para cada uno de los tipos de agrupación y para cada momento
...
Si bien existe disperción en los resultados, la realción que se había descrito
...
disminuye, per dicha disminución es mucho más marcada mientras menos precisa
sea la agrupación en términos de acercar personas contagiadas y acercar personas
no contagiadas. En este sentido podemos pensar en que el caso si agrupar es
...
Es imporante notar que tanto para el caso donde se conoce un $p_0$ o bien si 
...
de haber contraido el virus y se les aplica la prueba. Tomando los datos de las
...
pruebas necesarias en un orden de magnitud de $40\%$ o más en depedencia de qué tan
efectiva sea la identificación de $p_{ij}$ para cada indivíduo.
...
posibilidad de realizar el procedimiento para estas pruebas, es especial debido
a que la tendencia de $p_0$ en las muestras de la CCSS es negativa. Además, 
pareciera que en el caso de una reapertura tener capacidad de descartar
rápidamente grandes grupos de personas pareciera útil para el control 
epidemiológico.  Además, recientemente se indicó que el AyA se encuentra 
realizando pruebasde presencia/ausencia de COVID19 en las aguas residuales. 
Esto pareciera indicar que de alguna forma factible identificar en bajas 
concentraciones si el virus se encuentra o no en una muestra.
...
Además del desarrollo anteriormente hay muchas preguntas que pareciera 
...
* ¿Qué tan factible es generar una función de probabilidad en función de 
probabilidad de infección con base en los datos disponibles en la CCSS?
...
personas colaboraron tanto en recomendaciones respecto al concepto general 
